...
public:
	explicit Bootloader(bool sikRadio, QObject * = nullptr);

	QString errorString(void) { return _errorString; }

	bool open(const QString portName);
	void close(void) { device->port->close(); }
...
signals:
	...
	void updateProgress(const uint &);
...
private:
	...
	static const int _rebootTimeout                     =
		10000;    ///< Msecs to wait for reboot command to cause serial port to disconnect
	...
	quint32 crc32(const quint8 *, unsigned, unsigned);
};
