/****************************************************************************
 *
 * (c) 2009-2020 QGROUNDCONTROL PROJECT <http://www.agcs.org>
 *
 * AGCS is licensed according to the terms in the file
 * COPYING.md in the root of the source code directory.
 *
 ****************************************************************************/

#include "FirmwareImage.h"
//#include "AGCSLoggingCategory.h"
//#include "JsonHelper.h"
//#include "AGCSMAVLink.h"
//#include "AGCSApplication.h"
...
//    }
	...
	keys << _jsonBoardIdKey << _jsonParamXmlSizeKey << _jsonParamXmlKey << _jsonAirframeXmlSizeKey << _jsonAirframeXmlKey <<
	     _jsonImageSizeKey << _jsonImageKey << _jsonMavAutopilotKey;
...
//    }
	...
	if (!isCompatible(_boardId, firmwareBoardId)) {
		emit statusMessage(tr("Downloaded firmware board id does not match hardware board id: %1 != %2").arg(
					   firmwareBoardId).arg(_boardId));
		return false;
	}
...
//    emit statusMessage(QString("MAV_AUTOPILOT = %1").arg(firmwareType));
	...
	if (success) {
//        QString parameterFilename = AGCSApplication::cachedParameterMetaDataFile();
//        QFile parameterFile(AGCSApplication::cachedParameterMetaDataFile());
...
//        CompInfoParam::_cachePX4MetaDataFile(parameterFilename);
	...
	if (success) {
//        QString airframeFilename = AGCSApplication::cachedAirframeMetaDataFile();
		//qDebug() << airframeFilename;
//        QFile airframeFile(AGCSApplication::cachedAirframeMetaDataFile());
...
//        }
	...
	QString decompressFilename = imageDir.filePath("flash.bin");

	QFile decompressFile(decompressFilename);

	if (!decompressFile.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
		emit statusMessage(tr("Unable to open decompressed file %1 for writing, error: %2").arg(decompressFilename,
		...
		QByteArray		&decompressedBytes)	///< Returned decompressed bytes
{
	...
	if (decompressedBytes.count() != decompressedSize) {
		emit statusMessage(tr("Size for decompressed %1 does not match stored size: Expected(%1) Actual(%2)").arg(
					   decompressedSize).arg(decompressedBytes.count()));
		return false;
	}
