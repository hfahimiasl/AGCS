...
QGCCacheWorker::_runTask(QGCMapTask *task)
{
    ...
    qWarning() << "_runTask given unhandled task type" << task->type();
#endif
}
...
#ifdef DEBUG_LOCATION_PLUGIN
                qDebug() << "_deleteBingNoTileTiles HASH:" << query.value(2).toString();
#endif
...
#ifdef DEBUG_LOCATION_PLUGIN
        qWarning() << "_deleteBingNoTileTiles query failed";
#endif
    }
}
...
#ifdef DEBUG_LOCATION_PLUGIN
            qDebug() << "_saveTile() HASH:" << task->tile()->hash();
#endif
...
QGCCacheWorker::_getTile(QGCMapTask* mtask)
{
...
#ifdef DEBUG_LOCATION_PLUGIN
            qDebug() << "_getTile() (Found in DB) HASH:" << task->hash();
#endif
    ...
    if(!found) {
#ifdef DEBUG_LOCATION_PLUGIN
        qDebug() << "_getTile() (NOT in DB) HASH:" << task->hash();
#endif
...
QGCCacheWorker::_getTileSets(QGCMapTask* mtask)
{
    ...
    if(query.exec(s)) {
        while(query.next()) {
            ...
            set->setType(MapEngine::instance()->urlFactory()->getTypeByID(query.value("type").toInt()));
...
QGCCacheWorker::_updateSetTotals(QGCCachedTileSet* set)
{
    ...
    if(subquery.exec(sq)) {
        if(subquery.next()) {
            ...
            quint64 avg = MapEngine::instance()->urlFactory()->averageSizeForType(set->type());
...
QGCCacheWorker::_createTileSet(QGCMapTask *mtask)
{
    if(_valid) {
        ...
        query.addBindValue(MapEngine::instance()->urlFactory()->getIDByType(task->tileSet()->type()));
        ...
        if(!query.exec()) {
            ...
            for(int z = task->tileSet()->minZoom(); z <= task->tileSet()->maxZoom(); z++) {
                MapTileSet_t set = MapEngine::getTileCount(z,
                ...
                for(int x = set.tileX0; x <= set.tileX1; x++) {
                    for(int y = set.tileY0; y <= set.tileY1; y++) {
                        ...
                        QString hash = MapEngine::getTileHash(type, x, y, z);
                        ...
                        if(!tileID) {
                            ...
                            query.addBindValue(MapEngine::instance()->urlFactory()->getIDByType(type));
...
#ifdef DEBUG_LOCATION_PLUGIN
                            qDebug() << "_createTileSet() Already Cached HASH:" << hash;
#endif
                        }
                    }
                }
...
QGCCacheWorker::_getTileDownloadList(QGCMapTask* mtask)
{
    ...
    if(query.exec(s)) {
        while(query.next()) {
            ...
            tile->setType(MapEngine::instance()->urlFactory()->getTypeByID(query.value("type").toInt()));
...
#ifdef DEBUG_LOCATION_PLUGIN
            qDebug() << "_pruneCache() HASH:" << query.value(2).toString();
#endif
        }
        while(tlist.count()) {
                        ...
                        if(subQuery.exec(sb)) {
                            ...
                            while(subQuery.next()) {
                                ...
                                if(cQuery.exec()) {
                                    ...
                                    currentCount++;
                                    if(tileCount) {
                                        int progress = int(double(currentCount) / tileCount * 100.0);
    ...
    if (dbExport->open()) {
        if(_createDB(*dbExport, false)) {
            ...
            for(int i = 0; i < task->sets().count(); i++) {
                ...
                exportQuery.addBindValue(MapEngine::instance()->urlFactory()->getIDByType(set->type()));
                            ...
                            if(subQuery.exec(s)) {
                                if(subQuery.next()) {
                                    ...
                                    if(exportQuery.exec()) {
                                        ...
                                        exportQuery.exec();
                                        currentCount++;
                                        task->setProgress(int(double(currentCount) / tileCount * 100.0));
                                    }
                                }
                            }
                        }
...
#ifdef DEBUG_LOCATION_PLUGIN
            qDebug() << "No internet connection, reason:" << error;
#else
            Q_UNUSED(error)
#endif
