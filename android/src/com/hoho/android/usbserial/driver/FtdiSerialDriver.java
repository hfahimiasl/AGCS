...
import org.adlanpaya.agcs.AGCSActivity;
...
public class FtdiSerialDriver extends CommonUsbSerialDriver {
    ...
    public void open() throws IOException {
        ...
        try {
            ftD2xx = D2xxManager.getInstance(AGCSActivity.m_context);
        } catch (D2xxManager.D2xxException ex) {
            AGCSActivity.agcsLogDebug("D2xxManager.getInstance threw exception: " + ex.getMessage());
        }

        if (ftD2xx == null) {
            String errMsg = "Unable to retrieve D2xxManager instance.";
            AGCSActivity.agcsLogWarning(errMsg);
            throw new IOException(errMsg);
        }
        AGCSActivity.agcsLogDebug("Opened D2xxManager");

        int DevCount = ftD2xx.createDeviceInfoList(AGCSActivity.m_context);
        AGCSActivity.agcsLogDebug("Found " + DevCount + " ftdi devices.");
        if (DevCount < 1) {
        ...
        m_ftDev = null;
        try {
            m_ftDev = ftD2xx.openByIndex(AGCSActivity.m_context, 0);
        } catch (NullPointerException e) {
            AGCSActivity.agcsLogDebug("ftD2xx.openByIndex exception: " + e.getMessage());
        } finally {
            if (m_ftDev == null) {
                throw new IOException("No FTDI Devices found");
            }
        }
        AGCSActivity.agcsLogDebug("Opened FTDI device.");
    }
    ...
    public void close() {
        if (m_ftDev != null) {
            try {
                ...
                AGCSActivity.agcsLogWarning("close exception: " + e.getMessage());
            }
            m_ftDev = null;
        }
    }
    ...
    public int read(byte[] dest, int timeoutMillis) throws IOException {
        ...
        if (bytesAvailable > 0) {
            ...
            try {
                ...
                AGCSActivity.agcsLogWarning(errorMsg);
    ...
    public int write(byte[] src, int timeoutMillis) throws IOException {
        try {
            ...
            AGCSActivity.agcsLogWarning("Error writing: " + e.getMessage());
        }
        return 0;
    }
    ...
    private int setBaudRate(int baudRate) throws IOException {
        try {
            ...
            AGCSActivity.agcsLogWarning("Error setting baud rate: " + e.getMessage());
        }
        return 0;
    }
    ...
    public void setParameters(int baudRate, int dataBits, int stopBits, int parity) throws IOException {
        ...
        try {
            ...
            AGCSActivity.agcsLogWarning("Error setDataCharacteristics: " + e.getMessage());
        }
    }
    @Override
