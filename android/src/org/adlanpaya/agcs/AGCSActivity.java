...
//  The code uses the usb-serial-for-android software library.  The AGCSActivity class is the
...
public class AGCSActivity extends QtActivity
{
    public  static int                                  BAD_DEVICE_ID = 0;
    private static AGCSActivity                          _instance = null;
    ...
    private static final String                         TAG = "AGCS_AGCSActivity";
    private static PowerManager.WakeLock                _wakeLock;
    private static final String                         ACTION_USB_PERMISSION = "org.adlanpaya.agcs.action.USB_PERMISSION";
                    ...
                    synchronized (_instance) {
                        ...
                        if (device != null) {
                            ...
                            if (intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)) {
                                agcsLogDebug("Permission granted to " + device.getDeviceName());
                                driver.setPermissionStatus(UsbSerialDriver.permissionStatusSuccess);
                            } else {
                                agcsLogDebug("Permission denied for " + device.getDeviceName());
    ...
    public static native void agcsLogDebug(String message);
    public static native void agcsLogWarning(String message);

    public native void nativeInit();

    // AGCSActivity singleton
    public AGCSActivity()
    {
        _instance =                 this;
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        _wakeLock = pm.newWakeLock(PowerManager.SCREEN_BRIGHT_WAKE_LOCK, "AGCS");
        if(_wakeLock != null) {
            _wakeLock.acquire();
        } else {
	...
	if (_wifiMulticastLock == null)
            {
                ...
                _wifiMulticastLock = wifi.createMulticastLock("AGCS");
    ...
    private static void updateCurrentDrivers()
    {
        ...
        for (int i=_drivers.size()-1; i>=0; i--) {
            ...
            if (!found) {
                agcsLogDebug("Remove stale driver " + _drivers.get(i).getDevice().getDeviceName());
                _drivers.remove(i);
            }
        }
        ...
        for (int i=0; i<currentDrivers.size(); i++) {
            ...
            if (!found) {
                ...
                agcsLogDebug("Adding new driver " + deviceName);

                // Request permission if needed
                if (_usbManager.hasPermission(device)) {
                    agcsLogDebug("Already have permission to use device " + deviceName);
                    newDriver.setPermissionStatus(UsbSerialDriver.permissionStatusSuccess);
                } else {
                    agcsLogDebug("Requesting permission to use device " + deviceName);
    ...
    public static int open(Context parentContext, String deviceName, long userData)
    {
        ...
        if (driver == null) {
            agcsLogWarning("Attempt to open unknown device " + deviceName);
            return BAD_DEVICE_ID;
        }

        if (driver.permissionStatus() != UsbSerialDriver.permissionStatusSuccess) {
            agcsLogWarning("Attempt to open device with incorrect permission status " + deviceName + " " + driver.permissionStatus());
            return BAD_DEVICE_ID;
        }
        ...
        try {
            ...
            agcsLogDebug("Port open successful");
            ...
            agcsLogWarning("Port open exception: " + exA.getMessage());
            return BAD_DEVICE_ID;
        }
